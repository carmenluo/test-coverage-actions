...
describe("functions", () => {
  it("fails on invalid file", async () => {
    ...
    await expect(parser.readFile(filename)).rejects.toThrow(
  ...
  it("calculates level", async () => {
    ...
    [
      ...
      [89, 50, 90, "yellow"],
      [90, 50, 90, "green"],
    ].forEach(([linesRate, thresholdAlert, thresholdWarning, level]) => {
  ...
  it("generates badge URL", async () => {
    ...
    expect(parser.generateBadgeUrl(metric)).toStrictEqual(
  ...
  it("generates header", async () => {
    ...
    expect(
      parser.generateCommentHeader({ commentContext: "foobar" })
    ).toStrictEqual(`<!-- coverage-monitor-action: foobar -->`);
  });
...
`;
    ...
    expect(
      parser.generateTable({ metric, commentContext: "Coverage Report" })
          ...
          name
            ...
            .reduce(
              (carry, item) =>
                carry === null
                  ? item
                  : `${carry}${item[0].toUpperCase() + item.slice(1)}`,
              null
            )
        ];
      },
  ...
  Object.entries({
    ...
    "on missing number": {
      payload: {
        pull_request: { html_url: "https://example.com", head: { sha: "foo" } },
      },
    },
    "on missing pull request URL": {
      payload: { pull_request: { number: 1234, head: { sha: "foo" } } },
    },
    "on missing head info": {
      payload: {
        pull_request: { number: 1234, html_url: "https://example.com" },
      },
    },
    "on invalid head sha": {
      payload: {
    ...
    it(`fails on invalid webhook request: ${dataset}`, async () => {
      expect.hasAssertions();
      expect(() => {
        parser.parseWebhook(request);
      }).toThrow(new Error("Action supports only pull_request event"));
    });
  });
  ...
  it("parses webhook request", async () => {
    ...
    const { prNumber, prUrl, sha } = parser.parseWebhook({
      payload: {
        pull_request: {
          number: 1234,
          html_url: "https://example.com",
